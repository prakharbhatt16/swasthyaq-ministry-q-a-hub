# SwasthyaQ — Ministry Q&A Hub

[![[cloudflarebutton]]](https://deploy.workers.cloudflare.com/?url=${repositoryUrl})

SwasthyaQ is a visually polished, local-deployable Q&A management web application designed for the Ministry of Health. It enables users to create, categorize, and manage parliamentary-style questions by division, attach hyperlinks to external folders (e.g., shared drives), update question statuses, and visualize metrics through interactive dashboards. Built on Cloudflare Workers with Durable Objects for persistent storage, the app provides a responsive, modern interface inspired by Azure DevOps workflows, complete with CRUD operations, real-time-like updates, and professional-grade UI polish.

## Features

- **Question Management**: Create, edit, view, and delete questions with rich text support, categorized by health ministry divisions (e.g., AYUSH, Family Welfare).
...
- **Search & Filtering**: Full-text search, division/status filters, pagination, and grid/list views for questions.
...
1. **Create a Question**:
   ...
   - Fill title, select division (e.g., "Health"), add body (rich text), attach folder links.
...
1. **New Entity** (e.g., Questions):
   - In `shared/types.ts`: Define `Question { id: string; title: string; division: string; status: 'Draft' | ...; attachments: string[]; ... }`.
