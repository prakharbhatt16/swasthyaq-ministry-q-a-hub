...
export default function AdminPage() {
  ...
  const exportMutation = useMutation({
    mutationFn: async ({ format }: { format: 'csv' | 'excel' }) => {
      if (format === 'csv') {
        // Backend now returns JSON-wrapped CSV string
    ...
    onSuccess: ({ data, format }) => {
      ...
      toast.success(`Full ${format.toUpperCase()} export completed`);
    },
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
          <Card className="w-full max-w-sm">
            ...
            <CardContent className="space-y-4">
              ...
              />
              <Button onClick={handleLogin} className="w-full bg-[#F38020] hover:bg-[#d86d11] text-white">
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>This will reset all questions and attachments to the initial demo state. This action cannot be undone.</AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button disabled={exportMutation.isPending}>
                        ...
                        Export Data <ChevronDown className="h-4 w-4 ml-2" />
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent>
                      <DropdownMenuItem onSelect={() => exportMutation.mutate({ format: 'csv' })}>Export as CSV</DropdownMenuItem>
                      <DropdownMenuItem onSelect={() => exportMutation.mutate({ format: 'excel' })}>Export as Excel</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </CardContent>
              </Card>
