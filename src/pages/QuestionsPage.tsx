...
export default function QuestionsPage() {
  ...
  const allOnPageSelected = useMemo(() => { 
    ...
    return items.length > 0 && items.every(q => selectedIds.has(q.id)); 
  }, [data?.items, selectedIds]);
  const toggleSelectAll = () => setSelectedIds(prev => { 
    const newSelected = new Set(prev); 
    const items = data?.items ?? []; 
    if (allOnPageSelected) items.forEach(q => newSelected.delete(q.id)); 
    else items.forEach(q => newSelected.add(q.id)); 
    return newSelected; 
  });
  ...
  const bulkUpdateMutation = useMutation({
    ...
    onMutate: () => {
      toast.loading('Updating questions...', { id: 'bulk-update' });
    },
    onSuccess: () => { 
      toast.success('Questions updated successfully', { id: 'bulk-update' }); 
      queryClient.invalidateQueries({ queryKey: ['questions'] }); 
      queryClient.invalidateQueries({ queryKey: ['metrics'] }); 
      setSelectedIds(new Set()); 
    },
    onError: (err) => toast.error(`Update failed: ${err.message}`, { id: 'bulk-update' })
  });
  const exportMutation = useMutation({
    mutationFn: async ({ format }: { format: 'csv' | 'excel' }) => {
      if (format === 'csv') {
        // Now returns JSON string via api() helper
      ...
      <div className="text-center py-16 border-2 border-dashed rounded-lg">
        ...
        <div className="mt-6">
          <Button asChild className="bg-[#F38020] hover:bg-[#d86d11] text-white">
            <Link to="/questions/new"><PlusCircle className="mr-2 h-4 w-4" /> Create New Question</Link>
          </Button>
        </div>
      </div>
    );
          ...
          <div className="mb-6 p-4 bg-card rounded-lg shadow-sm space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-center">
              <div className="relative w-full lg:col-span-2">
                ...
                <Input 
                  placeholder="Search title, ticket, member, tags..." 
                  className="pl-10" 
                  value={searchTerm} 
              ...
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                ...
                <SelectContent>
                  ...
                  {(['Draft', 'Submitted', 'Admitted', 'Non-Admitted', 'Answered', 'Closed'] as QuestionStatus[]).map(s => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Select value={tagFilter} onValueChange={setTagFilter}>
                ...
                <SelectContent>
                  ...
                  {tagsData?.tags.map(tag => <SelectItem key={tag} value={tag}>#{tag}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <AnimatePresence>
              {selectedIds.size > 0 && (
                <motion.div 
                  ...
                  exit={{ opacity: 0, y: -10 }} 
                  className="bg-primary/5 dark:bg-primary/10 p-3 rounded-md flex items-center justify-between"
                >
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent>
                        {(['Draft', 'Submitted', 'Admitted', 'Non-Admitted', 'Answered', 'Closed'] as QuestionStatus[]).map(status => (
                          <DropdownMenuItem 
                            key={status} 
                            onSelect={() => bulkUpdateMutation.mutate({ ids: Array.from(selectedIds), status })}
                          >
